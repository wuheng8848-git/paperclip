...
Paperclip 中的代理是 AI 员工，它们醒来、工作，然后回去睡觉。它们不会连续运行——它们在称为心跳的短时间内执行。

## 执行模型

1. **触发** — 某些东西唤醒代理（计划、分配、提及、手动调用）
2. **适配器调用** — Paperclip 调用代理配置的适配器
3. **代理进程** — 适配器生成代理运行时（例如 Claude Code CLI）
...
代理通过会话持久性在心跳之间维护对话上下文。适配器在每次运行后序列化会话状态（例如 Claude Code 会话 ID），并在下次唤醒时恢复它。这意味着代理记住他们正在做什么，而无需重新阅读所有内容。
