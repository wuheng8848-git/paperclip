...
Paperclip 的执行策略系统确保任务以适当的监督级别完成。它不是依靠代理记住将工作移交给审查，而是**运行时强制执行**审查和批准阶段。

## 概览

执行策略是任何问题上可选的结构化对象，定义了执行者完成工作后必须发生的事情。它支持三层执行：

| 层 | 目的 | 范围 |
|---|---|---|
| **需要评论** | 每个代理运行必须向问题发回评论 | 运行时不变量（始终开启） |
| **审查阶段** | 审查者检查质量/正确性并可以要求更改 | 每问题，可选 |
| **批准阶段** | 经理/利益相关者给出最终签署 | 每问题，可选 |

这些层组合在一起。问题可以只有审查、只有批准、两者按顺序、或两者都没有（只是需要评论的后备）。

## 数据模型

### 执行策略（问题字段：`executionPolicy`）
...
参与者可以是代理或董事会用户。每个阶段可以有多个参与者；运行时选择第一个符合条件的参与者，优先考虑任何明确请求的负责人，同时排除原始执行者。

### 执行状态（问题字段：`executionState`）
...
1. **问题创建**，`executionPolicy` 指定审查阶段（例如，QA）和批准阶段（例如，CTO）。
2. **执行者工作** 处于 `进行中` 状态。
...
独立于审查阶段，每个问题绑定的代理运行必须留下评论。这在运行时级别强制执行：

1. **运行完成** — 运行时检查代理是否为此运行发布了评论。
2. **如果没有评论**：`issueCommentStatus` 设置为 `retry_queued`，代理以原因 `missing_issue_comment` 再次唤醒。
3. **如果重试后仍然没有评论**：`issueCommentStatus` 设置为 `retry_exhausted`。不再重试。失败被记录。
4. **如果发布了评论**：`issueCommentStatus` 设置为 `satisfied` 并链接到评论 ID。

这可以防止代理完成工作但没有留下任何痕迹的静默完成。
...
如果在审查进行中删除策略（`null`），执行状态将被清除，问题将返回给原始执行者。

### 推进阶段（审查者/批准者批准）

活动的审查者或批准者将问题转换为 `已完成` 并附评论：
...
创建新问题时，**审查者**和**批准者**按钮与负责人选择器一起出现。单击任一按钮会打开参与者选择器，其中包含：
- "无审查者" / "无批准者"（清除）
- "我"（当前用户）
- 代理和董事会用户的完整列表
...
对于现有问题，属性面板显示可编辑的**审查者**和**批准者**字段。每个阶段可以添加多个参与者。更改通过 API 保存到问题的 `executionPolicy`。

## 设计原则

1. **运行时强制执行，不依赖提示。** 代理不需要记住移交工作。运行时拦截状态转换并相应路由。
2. **迭代式，而非终端式。** 审查是一个循环（要求更改 → 修改 → 重新审查），而不是一次性门控。系统在重新提交时返回到同一阶段。
3. **灵活的角色。** 参与者可以是代理或用户。并非每个组织都有"QA"——审查者/批准者模式足够通用，适用于同行审查、经理签署、合规检查或多参与方工作流。
4. **可审计。** 每个决定都记录了操作者、结果、评论和运行 ID。完整的审查历史记录可按问题查询。
5. **保持单一执行不变量。** 审查唤醒和评论重试尊重现有的约束，即每个问题一次只能有一个代理运行处于活动状态。