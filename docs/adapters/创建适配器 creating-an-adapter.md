...
多数情况下，**应构建外部适配器插件**：边界更清晰、可独立发版，且无需改 Paperclip 源码。完整说明见 [外部适配器](/adapters/外部适配器%20external-adapters)。
...
my-adapter/                   # 外部插件
  ...
  src/
    ...
    ui-parser.ts        # 自包含 UI 解析器（外部——见 [UI 解析器契约](/adapters/适配器%20UI%20解析器%20adapter-ui-parser)）
    cli/
      index.ts          # CLI 导出
...
外部适配器改用自包含的 `ui-parser.ts`，见 [UI 解析器契约](/adapters/适配器%20UI%20解析器%20adapter-ui-parser)。
...
- [外部适配器](/adapters/外部适配器%20external-adapters) — 构建独立适配器插件
- [UI 解析器契约](/adapters/适配器%20UI%20解析器%20adapter-ui-parser) — 自定义运行日志解析
