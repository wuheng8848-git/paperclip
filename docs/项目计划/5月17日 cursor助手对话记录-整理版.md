...
> **口径提示（2026-05-20）：** 下文「编排注入」在现行 Board 上已改名为 **「运行清单」**（侧边栏 **工作** 分组内）。写新执行单、助手步骤请以 **[023-实践-Board界面中文口径与文档对照表](最佳实践/023-实践-Board界面中文口径与文档对照表.md)** 为准，勿批量改本历史正文。
...
- **验尸协作稿**：不往 `docs/` 对外树乱塞 → 优先 **`docs/项目计划/执行/`** 事务内「验证/证据」小节，或 **`探查/`** 未定责阶段，升格规则见 **`最佳实践/`**。
...
长时间、可控、不断流转的 AI 控制系统：**有状态可追溯、权限与上下文可见、事务签出交接循环**，非单次聊天。
