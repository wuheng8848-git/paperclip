...
**相关合并视角**（僵尸 run、反复唤醒、活动入口）：[`实践-工单心跳与僵尸run排障.md`](实践-工单心跳与僵尸run排障.md)

本文记录一次真实探查（**ROU-20**）时的**操作顺序与踩坑**，便于下次在几分钟内复现，而不在长 JSON 或错误字段名上耗时间。结论表见 **`../探查/探查-ROU-20-运行记录.md`**；`process_lost_retry` 语义见 **`../探查/探查-process_lost_retry.md`**；**合订（行为 + 指南 + 反查表 + 反复触发）**见 **`../探查/探查-ROU-20-process_lost-retry与反复触发-合订.md`**。下文 **§1.5** 记录「同一条工单在 Board 上混杂三类来源」的判读观察。
...
| 判读 `process_lost_retry` 是否「重复误排队」 | 需看 **`retryOfRunId`** 与 **原 run 的 `wakeReason`** | 对每条 `process_lost_retry` 拉详情 + 再拉 `retryOfRunId` 指向的原 run（或对照 `../探查/探查-process_lost_retry.md` 的「最多一次重试」规则）。 |
...
| 仅排队/运行中 | `GET /api/companies/{companyId}/live-runs?limit=50`（探查时注意 **`minCount`** 会掺历史，见 `../探查/探查-ROU-21-完成后反复唤醒.md`） |
