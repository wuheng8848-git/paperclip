# 实践：事务、心跳 run、僵尸 run 排障

把 **`执行/`** 里分散的「怎么收尾、怎么取证」收成一页；个案数据与长表仍在 **探查/**。

**编排定稿（改代码准绳）：** [`../编排/系统逻辑/事务运行结束编排/关单后运行结案对账.md`](../编排/系统逻辑/事务运行结束编排/关单后运行结案对账.md)
...
## 1. 僵尸 run（事务已完，run 仍 `running` / `queued`）
...
3. **判读速查表**：同探查文 §「判读速查」。
4. **脚本聚合**：仓库根 `pnpm issue:forensics -- --company <uuid> --issue <ref> ...`；**`--base` 默认 `http://127.0.0.1:3100`，不要写成带 `/api` 的根**，否则会打成 `/api/api/...`。全文命令表见 [002-实践-事务运行记录API取证路径.md](002-实践-事务运行记录API取证路径.md)。
