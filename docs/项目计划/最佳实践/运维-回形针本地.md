...
2. **任意终端，仓库根目录**（收尾、或找不到原窗口时）  
   ...
   **仍怀疑子进程泄漏**（未登记的 `node`、内嵌 `postgres.exe`、占 3100/5173 等）：仓库根一键 **`pnpm dev:nuke`**（只预览：`pnpm dev:nuke -- -DryRun`；保留 Codebuddy：`pnpm dev:nuke -- -KeepCodebuddy`）。与根目录 **`AGENTS.md`** 一致。
