...
> **本实例使用外部 PostgreSQL（Docker 容器），端口 4100。** 详见 [数据库](/03 部署/05%20数据库%20database.md)。
...
这会启动 `http://localhost:3100`（上游嵌入式 PG 默认端口；**本仓库 Routic 实例见文首 4100**）。
...
有关完整设置和故障排除，请参阅 [Tailscale 私有访问](/03 部署/09%20Tailscale%20私有访问%20tailscale-private-access)。
