...
export function NewProjectDialog() {
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) throw new Error(newProjectDialog.noCompanySelected);
  ...
  const deriveWorkspaceNameFromPath = (value: string) => {
    ...
    return segments[segments.length - 1] ?? newProjectDialog.workspaceNameLocalFallback;
  };
  ...
  const deriveWorkspaceNameFromRepo = (value: string) => {
    try {
      ...
      return repo || newProjectDialog.workspaceNameRepoFallback;
    } catch {
      return newProjectDialog.workspaceNameRepoFallback;
    }
  };
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{newProjectDialog.header}</span>
          </div>
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <input
            ...
            placeholder={newProjectDialog.namePlaceholder}
            value={name}
        ...
        <div className="px-4 pb-2">
          <MarkdownEditor
            ...
            placeholder={newProjectDialog.descriptionPlaceholder}
            bordered={false}
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{newProjectDialog.repoUrlLabel}</label>
              <span className="text-xs text-muted-foreground/50">{newProjectDialog.optional}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {newProjectDialog.repoUrlTooltip}
                </TooltipContent>
              </Tooltip>
            </div>
            <input
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{newProjectDialog.localFolderLabel}</label>
              <span className="text-xs text-muted-foreground/50">{newProjectDialog.optional}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {newProjectDialog.localFolderTooltip}
                </TooltipContent>
              </Tooltip>
            </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap">
          ...
          <Popover open={statusOpen} onOpenChange={setStatusOpen}>
            ...
            <PopoverContent className="w-40 p-1" align="start">
              {PROJECT_CREATE_STATUS_VALUES.map((value) => (
                <button
                  key={value}
                  className={cn(
                ...
                >
                  {formatProjectPropertyStatusLabel(value)}
                </button>
              ))}
            </PopoverContent>
          </Popover>
          ...
          {selectedGoals.map((goal) => (
            ...
            >
              ...
              <button
                ...
                aria-label={newProjectDialog.removeGoalAria(goal.title)}
                type="button"
              >
                <X className="h-3 w-3" />
              </button>
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? newProjectDialog.addGoalChip : newProjectDialog.goalChip}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-56 p-1" align="start">
              ...
              {selectedGoals.length > 0 && availableGoals.length === 0 && (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  {newProjectDialog.allGoalsSelected}
                </div>
              )}
            </PopoverContent>
          </Popover>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs">
            ...
            <input
              ...
              placeholder={newProjectDialog.targetDatePlaceholder}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{newProjectDialog.createFailed}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? newProjectDialog.creatingEllipsis : newProjectDialog.createProject}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
