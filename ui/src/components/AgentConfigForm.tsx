...
import { agentConfigUi, agentDetailUi } from "../lib/i18n";
...
const codexThinkingEffortOptions = [
  { id: "", label: agentConfigUi.thinkingEffortAuto },
  { id: "minimal", label: agentConfigUi.thinkingEffortMinimal },
  { id: "low", label: agentConfigUi.thinkingEffortLow },
  { id: "medium", label: agentConfigUi.thinkingEffortMedium },
  { id: "high", label: agentConfigUi.thinkingEffortHigh },
  { id: "xhigh", label: agentConfigUi.thinkingEffortXHigh },
] as const;
...
const openCodeThinkingEffortOptions = [
  ...
  { id: "high", label: agentConfigUi.thinkingEffortHigh },
  { id: "xhigh", label: agentConfigUi.thinkingEffortXHigh },
  { id: "max", label: agentConfigUi.thinkingEffortMax },
] as const;

const cursorModeOptions = [
  { id: "", label: agentConfigUi.thinkingEffortAuto },
  { id: "plan", label: agentConfigUi.cursorModePlan },
  { id: "ask", label: agentConfigUi.cursorModeAsk },
] as const;
...
export function AgentConfigForm(props: AgentConfigFormProps) {
      ...
      errorMessage: testEnvironment.error instanceof Error
        ? testEnvironment.error.message
          : testEnvironment.error
            ? agentConfigUi.envTestFailedGeneric
          : null,
  ...
  async function handleRefreshModels() {
    ...
    try {
      ...
      setRefreshModelsError(error instanceof Error ? error.message : agentConfigUi.fallbackFailedRefreshModels);
    } finally {
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">{agentConfigUi.configUnsavedChanges}</span>
            <Button
              size="sm"
              onClick={handleSave}
              disabled={!isCreate && props.isSaving}
            >
              {!isCreate && props.isSaving ? agentDetailUi.savingEllipsis : agentDetailUi.saveButton}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{agentConfigUi.sectionIdentity}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{agentConfigUi.sectionIdentity}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={agentConfigUi.fieldName} hint={help.name}>
              <DraftInput
                value={eff("identity", "name", props.agent.name)}
                onCommit={(v) => mark("identity", "name", v)}
                immediate
                className={inputClass}
                placeholder={agentConfigUi.placeholderAgentName}
              />
            </Field>
            <Field label={agentConfigUi.fieldTitle} hint={help.title}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder={agentConfigUi.placeholderTitleExample}
              />
            </Field>
            <Field label={agentConfigUi.fieldReportsTo} hint={help.reportsTo}>
              <ReportsToPicker
            ...
            <Field label={agentConfigUi.fieldCapabilities} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "") ?? ""}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={agentConfigUi.placeholderCapabilities}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={agentConfigUi.fieldPromptTemplate} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={agentConfigUi.placeholderPromptRoleFraming}
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {agentConfigUi.bannerPromptHeartbeatCost}
                </div>
              </>
            )}
          </div>
      ...
      {environmentsEnabled ? (
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{agentConfigUi.sectionExecution}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{agentConfigUi.sectionExecution}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={agentConfigUi.fieldDefaultEnvironment}
              hint={agentConfigUi.hintDefaultExecutionTarget}
            >
              <select
                className={inputClass}
              ...
              >
                <option value="">{agentConfigUi.optionCompanyDefaultLocal}</option>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{agentConfigUi.sectionAdapter}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{agentConfigUi.sectionAdapter}</span>
          }
          {showInlineAdapterTestEnvironmentButton && (
            <Button
              ...
              disabled={testEnvironmentDisabled}
            >
              {testEnvironment.isPending ? agentConfigUi.testingEnvironmentEllipsis : agentConfigUi.testEnvironmentShort}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={agentConfigUi.fieldAdapterType} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={agentConfigUi.fieldWorkingDirDeprecated} hint={help.cwd}>
              <div className="flex items-center gap-2 rounded-md border border-border px-2.5 py-1.5">
                ...
                <DraftInput
                  ...
                  placeholder={agentConfigUi.placeholderProjectPathUnix}
                />
                <ChoosePathButton />
              </div>
            </Field>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{agentConfigUi.sectionPermissionsAndConfig}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{agentConfigUi.sectionPermissionsAndConfig}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={agentConfigUi.fieldCommandLine} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {supportsModelProfiles && (
                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{agentConfigUi.labelPrimaryModelRow}</div>
              )}
              <ModelDropdown
                models={models}
                ...
                detectModelLabel={agentConfigUi.detectModelAction}
                emptyDetectHint={agentConfigUi.emptyDetectNeedsManualVendor}
              />
                ...
                <p className="text-xs text-destructive">
                  {refreshModelsError
                    ?? (fetchedModelsError instanceof Error
                      ...
                      : agentConfigUi.fallbackFailedLoadModelsFallback)}
                </p>
              )}
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {agentConfigUi.openCodeNeedsLocalDiscovery(currentDefaultEnvironment.name)}
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {agentConfigUi.bannerCodexMinimalWithSearch}
                      </p>
                    )}
                </>
              )}
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={agentConfigUi.fieldBootstrapLegacy} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={agentConfigUi.placeholderBootstrapPrompt}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {agentConfigUi.bannerBootstrapDeprecation}
                  </div>
                </>
              )}
              ...
              <Field label={agentConfigUi.fieldExtraArgsComma} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={agentConfigUi.placeholderExtraArgsExample}
                />
              </Field>

              <Field label={agentConfigUi.fieldEnvironmentVariables} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={agentConfigUi.fieldTimeoutSeconds} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={agentConfigUi.fieldGraceInterruptSeconds} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              ...
              numberLabel={agentConfigUi.unitSecondsShort}
              numberPrefix={agentConfigUi.runHeartbeatEveryPrefix}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg" : "")}>
            <div className={cn(cards ? "p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              />
              <Field label={agentConfigUi.fieldCooldownSeconds} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={agentConfigUi.fieldMaxConcurrentRuns} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={agentConfigUi.toggleContinuationAfterCap}
                ...
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={agentConfigUi.fieldContinuationAttempts} hint={help.maxTurnContinuationMaxAttempts}>
                      <DraftNumberInput
                    ...
                    <Field label={agentConfigUi.fieldContinuationDelaySeconds} hint={help.maxTurnContinuationDelaySec}>
                      <DraftNumberInput
...
export function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const statusLabel =
    result.status === "pass"
      ? agentConfigUi.envTestStatusPass
      : result.status === "warn"
        ? agentConfigUi.envTestStatusWarn
        : agentConfigUi.envTestStatusFail;
  const statusClass =
    result.status === "pass"
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{agentConfigUi.envTestHintLabel} {check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">{agentConfigUi.adapterComingSoon}</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
function ExperimentalBadge() {
  return (
    <span className="shrink-0 rounded border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-medium leading-none text-amber-700 dark:text-amber-200">
      {agentConfigUi.adapterExperimentalPill}
    </span>
  );
}
...
function ModelDropdown({
  ...
  return (
    <Field label={agentConfigUi.fieldModelDropdown} hint={help.model}>
      <Popover
        open={open}
          ...
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ...
                : value
                  || (allowDefault
                    ? (defaultLabel ?? agentConfigUi.placeholderModelPickDefault)
                    : required
                      ? agentConfigUi.placeholderModelPickRequired
                      : agentConfigUi.placeholderModelPickOptional)}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? agentConfigUi.modelSearchPlaceholderTypedCreate : agentConfigUi.modelSearchPlaceholder}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? agentConfigUi.modelDetectRunning : detectedModel ? agentConfigUi.redetectModelAction : (detectModelLabel ?? agentConfigUi.detectModelAction)}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels ? agentConfigUi.refreshingModelsEllipsis : agentConfigUi.refreshModelCatalog}
            </button>
          )}
          {value && (!models.some((m) => m.id === value) || promotedModelIds.has(value)) && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-green-500/15 text-green-400 border border-green-500/20">
                {agentConfigUi.modelBadgeCurrent}
              </span>
            </button>
          )}
          {detectedModel && detectedModel !== value && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/20">
                {agentConfigUi.modelBadgeDetected}
              </span>
            </button>
          )}
          {detectedModelCandidates
              ...
              return (
                ...
                >
                  ...
                  <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-sky-500/15 text-sky-400 border border-sky-500/20">
                    {agentConfigUi.modelBadgeViaConfig}
                  </span>
                </button>
              );
            })}
          <div className="max-h-[240px] overflow-y-auto">
            {allowDefault && (
              ...
              >
                {agentConfigUi.placeholderModelPickDefault}
              </button>
            )}
            {canCreateManualModel && (
              ...
              >
                <span>{agentConfigUi.modelUseManualChoice}</span>
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? agentConfigUi.emptyDetectNeedsManualVendor)
                    : agentConfigUi.modelListNoMatches}
                </p>
              </div>
            )}
          </div>
...
function CheapModelSection({
  ...
  const placeholderHint = adapterDefaultModel
    ? agentConfigUi.hintAdapterDefaultLine(adapterDefaultModel)
    : agentConfigUi.hintNoCheapDefaultPrompt;
  return (
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{agentConfigUi.cheapProfileTitle}</div>
          <p className="text-xs text-muted-foreground">
            {agentConfigUi.cheapProfileSubtitle}
          </p>
        </div>
      ...
      {enabled && !model && adapterDefaultModel ? (
        <p className="text-[11px] text-muted-foreground">
          {agentConfigUi.noteCheapImplicitFallbackPrefix} <code>{adapterDefaultModel}</code>.
        </p>
      ) : null}
      {enabled && !model && !adapterDefaultModel ? (
        <p className="text-[11px] text-amber-500">
          {agentConfigUi.warningCheapUnset}
        </p>
      ) : null}
    </div>
  );
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={agentConfigUi.fieldThinkingEffort} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? agentConfigUi.thinkingEffortAuto}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
