...
import {
  projectPropertiesUi,
  formatProjectPropertyStatusLabel,
  formatWorkspaceRuntimeServiceStatusZh,
  formatWorkspaceRuntimeLifecycleZh,
} from "../lib/i18n";

const PROJECT_STATUSES = [
  { value: "backlog", label: projectPropertiesUi.statusBacklog },
  { value: "planned", label: projectPropertiesUi.statusPlanned },
  { value: "in_progress", label: projectPropertiesUi.statusInProgress },
  { value: "completed", label: projectPropertiesUi.statusCompleted },
  { value: "cancelled", label: projectPropertiesUi.statusCancelled },
] as const;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {formatProjectPropertyStatusLabel(status)}
        </button>
      </PopoverTrigger>
...
function ArchiveDangerZone({
  ...
  const actionVerb = isArchive ? projectPropertiesUi.archiveverb : projectPropertiesUi.unarchiveverb;

  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive ? projectPropertiesUi.archiveBlurb : projectPropertiesUi.unarchiveBlurb}
      </p>
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          <Loader2 className="h-3 w-3 animate-spin mr-1" />
          {isArchive ? projectPropertiesUi.archiving : projectPropertiesUi.unarchiving}
        </Button>
      ) : confirming ? (
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {projectPropertiesUi.archiveConfirmPrompt(actionVerb, project.name)}
          </span>
          <Button
            size="sm"
            variant="destructive"
          ...
          >
            {projectPropertiesUi.cancelverb}
          </Button>
        </div>
      ) : (
        <Button
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{projectPropertiesUi.archiveProject}</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{projectPropertiesUi.unarchiveProject}</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(projectPropertiesUi.selectCompanyForSecrets);
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(projectPropertiesUi.errLocalFolderAbsolute);
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(projectPropertiesUi.errRepoUrlGithub);
      return;
    }
    setWorkspaceError(null);
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? projectPropertiesUi.confirmClearLocalFromWorkspace
        : projectPropertiesUi.confirmDeleteWorkspaceLocal,
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? projectPropertiesUi.confirmClearRepoFromWorkspace
        : projectPropertiesUi.confirmDeleteWorkspaceRepo,
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={projectPropertiesUi.fieldName} state={fieldState("name")} />}>
          {onUpdate || onFieldUpdate ? (
            <DraftInput
              ...
              placeholder={projectPropertiesUi.projectNamePlaceholder}
            />
          ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={projectPropertiesUi.fieldDescription} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            <InlineEditor
              ...
              placeholder={projectPropertiesUi.addDescriptionPlaceholder}
              multiline
            />
          ) : (
            <p className="text-sm text-muted-foreground">
              {project.description?.trim() || projectPropertiesUi.noDescription}
            </p>
          )}
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={projectPropertiesUi.fieldStatus} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={projectPropertiesUi.fieldLead}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={projectPropertiesUi.fieldGoals} state={fieldState("goals")} />}
          alignStart
                ...
                >
                  ...
                  {(onUpdate || onFieldUpdate) && (
                    <button
                      ...
                      aria-label={projectPropertiesUi.removeGoalAria(goal.title)}
                    >
                      <X className="h-3 w-3" />
                    </button>
                  )}
            ...
            <Popover open={goalOpen} onOpenChange={setGoalOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {projectPropertiesUi.addGoalButton}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-56 p-1" align="start">
                {availableGoals.length === 0 ? (
                  <div className="px-2 py-1.5 text-xs text-muted-foreground">
                    {projectPropertiesUi.allGoalsLinked}
                  </div>
                ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={projectPropertiesUi.fieldEnv} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              {projectPropertiesUi.envHint}
            </p>
          </div>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={projectPropertiesUi.fieldCreated} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={projectPropertiesUi.fieldUpdated} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={projectPropertiesUi.fieldTargetDate} state="idle" />}>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>{projectPropertiesUi.codebase}</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                  aria-label={projectPropertiesUi.codebaseHelpAria}
                >
                  ?
                </button>
              </TooltipTrigger>
              <TooltipContent side="top">
                {projectPropertiesUi.codebaseTooltip}
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{projectPropertiesUi.repoEyebrow}</div>
              {codebase.repoUrl ? (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      {projectPropertiesUi.changeRepo}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon-xs"
                      onClick={clearRepoWorkspace}
                      aria-label={projectPropertiesUi.clearRepoAria}
                    >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">{projectPropertiesUi.notSet}</div>
                  <Button
                    variant="outline"
                    size="xs"
                    className="h-6 px-2"
            ...
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{projectPropertiesUi.localFolderEyebrow}</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">{projectPropertiesUi.paperclipManagedFolder}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder ? projectPropertiesUi.changeLocalFolder : projectPropertiesUi.setLocalFolder}
                  </Button>
                  {codebase.localFolder ? (
                    <Button
                      ...
                      aria-label={projectPropertiesUi.clearLocalFolderAria}
                    >
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  ) : null}
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {projectPropertiesUi.legacyWorkspacesNote}
              </div>
            )}
                  ...
                  >
                    <div className="min-w-0 space-y-0.5">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {formatWorkspaceRuntimeServiceStatusZh(service.status)}
                        </span>
                      </div>
                      <div className="text-[11px] text-muted-foreground">
                        {service.url ? (
                          ...
                          service.command ?? projectPropertiesUi.noUrl
                        )}
                      </div>
                    </div>
                    <div className="text-[10px] text-muted-foreground whitespace-nowrap">
                      {formatWorkspaceRuntimeLifecycleZh(service.lifecycle)}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-1.5 rounded-md border border-border p-2">
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder={projectPropertiesUi.localPathPlaceholder}
                />
                <ChoosePathButton />
              </div>
              <div className="flex items-center gap-2">
          ...
          {workspaceMode === "repo" && (
            <div className="space-y-1.5 rounded-md border border-border p-2">
              <input
                ...
                placeholder={projectPropertiesUi.repoUrlPlaceholder}
              />
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{projectPropertiesUi.errSaveWorkspace}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{projectPropertiesUi.errDeleteWorkspace}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{projectPropertiesUi.errUpdateWorkspace}</p>
          )}
        </div>
        ...
        {isolatedWorkspacesEnabled ? (
          <>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span>{projectPropertiesUi.executionWorkspaces}</span>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                      aria-label={projectPropertiesUi.executionWorkspacesHelpAria}
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top">
                    {projectPropertiesUi.executionWorkspacesTooltip}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{projectPropertiesUi.enableIsolatedCheckouts}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {projectPropertiesUi.enableIsolatedCheckoutsHint}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <ToggleSwitch
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled ? projectPropertiesUi.enabled : projectPropertiesUi.disabled}
                    </span>
                  )}
                </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{projectPropertiesUi.newIssuesDefaultIsolated}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {projectPropertiesUi.newIssuesDefaultIsolatedHint}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? projectPropertiesUi.hideAdvancedCheckout
                          : projectPropertiesUi.showAdvancedCheckout}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {projectPropertiesUi.hostManagedImplementationPrefix}<span className="text-foreground">Git worktree</span>
                        </div>
                        {environmentsEnabled ? (
                          <div>
                            <div className="mb-1 flex items-center gap-1.5">
                              <label className="flex items-center gap-2 text-xs text-muted-foreground">
                                <span>{projectPropertiesUi.fieldEnvironment}</span>
                            ...
                            >
                              <option value="">{projectPropertiesUi.noEnvironmentOption}</option>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{projectPropertiesUi.fieldBaseRef}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{projectPropertiesUi.fieldBranchTemplate}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{projectPropertiesUi.fieldWorktreeParentDir}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{projectPropertiesUi.fieldProvisionCommand}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{projectPropertiesUi.fieldTeardownCommand}</span>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {projectPropertiesUi.provisionTeardownFootnote}
                        </p>
                      </div>
                    ) : null}
                  </div>
        ...
        <>
          ...
          <div className="space-y-4 py-4">
            <div className="text-xs font-medium text-destructive uppercase tracking-wide">
              {projectPropertiesUi.dangerZone}
            </div>
            <ArchiveDangerZone
              project={project}
              onArchive={onArchive}
