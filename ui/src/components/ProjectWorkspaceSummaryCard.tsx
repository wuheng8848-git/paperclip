...
import { projectWorkspaceCardUi, formatExecutionWorkspaceStatusZh } from "../lib/i18n";
...
function workspaceKindLabel(kind: ProjectWorkspaceSummary["kind"]) {
  return kind === "execution_workspace" ? projectWorkspaceCardUi.kindExecution : projectWorkspaceCardUi.kindProject;
}
          ...
          <div className="min-w-0 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center rounded-full border border-border/70 bg-background px-2.5 py-1 text-xs text-muted-foreground">
                {projectWorkspaceCardUi.updatedPrefix} {timeAgo(summary.lastUpdatedAt)}
              </span>
              {summary.serviceCount > 0 ? (
                ...
                >
                  ...
                  />
                  {projectWorkspaceCardUi.servicesCount(summary.runningServiceCount, summary.serviceCount)}
                </span>
              ) : null}
              {summary.executionWorkspaceStatus ? (
                <span className="inline-flex items-center rounded-full border border-border/70 bg-background px-2.5 py-1 text-xs text-muted-foreground">
                  {formatExecutionWorkspaceStatusZh(summary.executionWorkspaceStatus)}
                </span>
              ) : null}
            </div>
            <Link
          ...
          >
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices ? projectWorkspaceCardUi.stopServices : projectWorkspaceCardUi.startServices}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed" ? projectWorkspaceCardUi.retryClose : projectWorkspaceCardUi.closeWorkspace}
              </Button>
            ) : null}
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            {summary.branchName ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{projectWorkspaceCardUi.branchEyebrow}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.branchName}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={projectWorkspaceCardUi.branchCopied}
                    >
                      {summary.branchName}
                    </CopyText>
                    <CopyText
                      text={summary.branchName}
                      ariaLabel={projectWorkspaceCardUi.copyBranchAria}
            ...
            {summary.cwd ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{projectWorkspaceCardUi.pathEyebrow}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.cwd}
                      title={summary.cwd}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={projectWorkspaceCardUi.pathCopied}
                    >
                      {truncatePath(summary.cwd)}
                    </CopyText>
                    <CopyText
                      text={summary.cwd}
                      ariaLabel={projectWorkspaceCardUi.copyPathAria}
                      className="mt-0.5 shrink-0 text-muted-foreground hover:text-foreground"
                      copiedLabel={projectWorkspaceCardUi.pathCopied}
                    >
                      <Copy className="h-3.5 w-3.5" />
            ...
            {summary.primaryServiceUrl ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{projectWorkspaceCardUi.serviceEyebrow}</div>
                  <a
        ...
        {summary.issues.length > 0 ? (
          <div className="space-y-2">
            <div className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
              {projectWorkspaceCardUi.linkedIssues}
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {hiddenIssueCount > 0 ? (
                ...
                >
                  {projectWorkspaceCardUi.moreLinked(hiddenIssueCount)}
                </Link>
              ) : null}
            </div>
          </div>
