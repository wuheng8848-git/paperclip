...
export function BlockedInboxView({
  ...
  if (error) {
    const message =
      error instanceof Error ? error.message : inboxUi.blockedLoadingErrorTitle;
    return (
      <div
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 space-y-1">
            <p className="text-sm font-medium">{inboxUi.blockedLoadingErrorTitle}</p>
            <p className="text-xs opacity-80">
              {inboxUi.blockedLoadingErrorBody} {message}
            </p>
          </div>
          <Button
            type="button"
            ...
            disabled={isFetching}
          >
            {isFetching ? inboxUi.retrying : inboxUi.retry}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      ...
      >
        ...
        <div className="space-y-1">
          <p className="text-sm font-medium text-foreground">{inboxUi.blockedEmptyTitle}</p>
          <p className="text-xs text-muted-foreground">
            {inboxUi.blockedEmptyBody}
          </p>
        </div>
      </div>
    );
  ...
  if (groups.length === 0) {
    return (
      <div className="space-y-3">
        ...
        >
          {inboxUi.blockedNoSearchResults}
        </div>
      </div>
    );
  }
