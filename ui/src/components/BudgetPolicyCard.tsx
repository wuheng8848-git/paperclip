...
import {
  budgetPolicySoftWarnLine,
  budgetPolicyUi,
  formatBudgetPolicyStatusBadge,
  formatBudgetScopeTypeZh,
  formatBudgetWindowKindLabel,
} from "../lib/i18n";
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGridCellClass = cn(
    isPlain ? "" : "rounded-xl border border-border/70 bg-black/[0.18] px-4 py-3",
  );

  const observedBudgetGrid = (
    <div className={isPlain ? "grid gap-6 sm:grid-cols-2" : "grid gap-3 sm:grid-cols-2"}>
      <div className={observedBudgetGridCellClass}>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {budgetPolicyUi.sectionObserved}
        </div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? budgetPolicyUi.pctOfLimit(summary.utilizationPercent) : budgetPolicyUi.noCapConfigured}
        </div>
      </div>
      <div className={observedBudgetGridCellClass}>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {budgetPolicyUi.sectionBudget}
        </div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : budgetPolicyUi.budgetDisabledDisplay}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {budgetPolicySoftWarnLine(summary.warnPercent, summary.paused, summary.pauseReason)}
        </div>
      </div>
    </div>
  );
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{budgetPolicyUi.remainingLabel}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : budgetPolicyUi.unlimited}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? budgetPolicyUi.pauseExecutionProject
          : budgetPolicyUi.pauseHeartbeatsScope}
      </div>
    </div>
  ) : null;
    ...
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      <div className="min-w-0 flex-1">
        ...
        <Input
          ...
          className="mt-2"
          inputMode="decimal"
          placeholder={budgetPolicyUi.budgetInputPlaceholder}
        />
      </div>
      <Button
        onClick={() => {
      ...
      >
        {isSaving ? budgetPolicyUi.savingButton : summary.amount > 0 ? budgetPolicyUi.updateBudgetButton : budgetPolicyUi.setBudgetButton}
      </Button>
    </div>
  ) : null;

  const statusBadgeText = formatBudgetPolicyStatusBadge(summary.paused, summary.status);
  ...
  if (isPlain) {
    return (
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground">
              {formatBudgetScopeTypeZh(summary.scopeType)}
            </div>
            <div className="mt-2 text-xl font-semibold">{summary.scopeName}</div>
            <div className="mt-2 text-sm text-muted-foreground">{formatBudgetWindowKindLabel(summary.windowKind)}</div>
          </div>
          <div
            className={cn(
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{budgetPolicyUi.invalidUsdInputHint}</p>
        ) : null}
      </div>
    );
  }
      ...
      <CardHeader className={cn("gap-3", compact ? "px-4 pt-4 pb-2" : "px-5 pt-5 pb-3")}>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1">{formatBudgetWindowKindLabel(summary.windowKind)}</CardDescription>
          </div>
