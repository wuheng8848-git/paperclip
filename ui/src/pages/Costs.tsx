...
import { costsUi, formatBudgetScopeTypeZh } from "../lib/i18n";
  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{costsUi.financeSummaryTitle}</CardTitle>
        <CardDescription>{costsUi.financeSummaryDescription}</CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={costsUi.financeSummaryDebits}
          value={formatCents(debitCents)}
          subtitle={costsUi.financeSummaryDebitsSubtitle(eventCount)}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={costsUi.financeSummaryCredits}
          value={formatCents(creditCents)}
          subtitle={costsUi.financeSummaryCreditsSubtitle}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={costsUi.financeSummaryNet}
          value={formatCents(netCents)}
          subtitle={costsUi.financeSummaryNetSubtitle}
          icon={ReceiptText}
        />
        <MetricTile
          label={costsUi.financeSummaryEstimated}
          value={formatCents(estimatedDebitCents)}
          subtitle={costsUi.financeSummaryEstimatedSubtitle}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: costsUi.pageTitle }]);
  }, [setBreadcrumbs]);
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{costsUi.allProviders}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={costsUi.emptySelectCompany} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{costsUi.pageTitle}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {costsUi.pageSubtitle}
                </p>
            </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{costsUi.customRangeTo}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={costsUi.metricInferenceSpend}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={costsUi.inferenceSpendSubtitle(formatTokens(inferenceTokenTotal))}
              icon={DollarSign}
            />
            <MetricTile
              label={costsUi.metricBudget}
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ? `${spendData.summary.utilizationPercent}%`
                  : costsUi.budgetMetricOpen
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? costsUi.budgetPausedSubtitle(budgetData?.pausedAgentCount ?? 0, budgetData?.pausedProjectCount ?? 0)
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? costsUi.budgetUtilSubtitle(
                        formatCents(spendData.summary.spendCents),
                        formatCents(spendData.summary.budgetCents),
                      )
                    : costsUi.budgetNoMonthlyCap
              }
              icon={Coins}
            />
            <MetricTile
              label={costsUi.metricFinanceNet}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={costsUi.financeNetSubtitle(
            ...
            />
            <MetricTile
              label={costsUi.metricFinanceEvents}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={costsUi.financeEventsSubtitle(
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{costsUi.tabOverview}</TabsTrigger>
          <TabsTrigger value="budgets">{costsUi.tabBudgets}</TabsTrigger>
          <TabsTrigger value="providers">{costsUi.tabProviders}</TabsTrigger>
          <TabsTrigger value="billers">{costsUi.tabBillers}</TabsTrigger>
          <TabsTrigger value="finance">{costsUi.tabFinance}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{costsUi.selectDatesToLoad}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{costsUi.inferenceLedgerTitle}</CardTitle>
                    <CardDescription>{costsUi.inferenceLedgerDescription}</CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? costsUi.budgetLine(formatCents(spendData.summary.budgetCents))
                            : costsUi.unlimitedBudget}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{costsUi.usageEyebrow}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {costsUi.monthlyBudgetConsumedPct(spendData.summary.utilizationPercent)}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{costsUi.byAgentTitle}</CardTitle>
                    <CardDescription>{costsUi.byAgentDescription}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{costsUi.noCostEvents}</p>
                    ) : (
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {costsUi.runCountApi(row.apiRunCount)}
                                    {" · "}
                                    {costsUi.runCountSubscription(row.subscriptionRunCount)}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{costsUi.byProjectTitle}</CardTitle>
                    <CardDescription>{costsUi.byProjectDescription}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{costsUi.noProjectAttributedCosts}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? costsUi.unattributed}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={costsUi.financeTimelineEmptyCostsOverview} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{costsUi.budgetControlPlaneTitle}</CardTitle>
                  <CardDescription>{costsUi.budgetControlPlaneDescription}</CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={costsUi.metricActiveIncidents}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={costsUi.activeIncidentsSubtitle}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={costsUi.metricPendingApprovals}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={costsUi.pendingApprovalsSubtitle}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={costsUi.metricPausedAgents}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={costsUi.pausedAgentsSubtitle}
                    icon={Coins}
                  />
                  <MetricTile
                    label={costsUi.metricPausedProjects}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={costsUi.pausedProjectsSubtitle}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{costsUi.activeIncidentsHeading}</h2>
                    <p className="text-sm text-muted-foreground">{costsUi.activeIncidentsHint}</p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">
                          {costsUi.scopeBudgetTitle(formatBudgetScopeTypeZh(scopeType))}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {costsUi.scopeBudgetDescription[scopeType]}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{costsUi.noCostEventsInPeriod}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{costsUi.noBillableEventsInPeriod}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{costsUi.financeByBillerTitle}</CardTitle>
                    <CardDescription>{costsUi.financeByBillerDescription}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{costsUi.noFinanceEvents}</p>
                      ) : (
