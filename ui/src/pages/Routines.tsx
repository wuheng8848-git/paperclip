...
import { nav, routinesPage, routinesShared } from "../lib/i18n";
...
export function buildRoutineGroups(
  ...
  if (groupByValue === "project") {
    ...
    return Object.keys(groups)
      .sort((left, right) => {
        const leftLabel = left === "__no_project" ? routinesShared.noProject : (projectById.get(left)?.name ?? routinesShared.unknownProject);
        const rightLabel = right === "__no_project" ? routinesShared.noProject : (projectById.get(right)?.name ?? routinesShared.unknownProject);
        return leftLabel.localeCompare(rightLabel);
      })
      .map((key) => ({
        key,
        label: key === "__no_project" ? routinesShared.noProject : (projectById.get(key)?.name ?? routinesShared.unknownProject),
        items: groups[key]!,
      }));
  }
  ...
  return Object.keys(groups)
    .sort((left, right) => {
      const leftLabel = left === "__unassigned" ? routinesShared.unassigned : (agentById.get(left)?.name ?? routinesShared.unknownAgent);
      const rightLabel = right === "__unassigned" ? routinesShared.unassigned : (agentById.get(right)?.name ?? routinesShared.unknownAgent);
      return leftLabel.localeCompare(rightLabel);
    })
    .map((key) => ({
      key,
      label: key === "__unassigned" ? routinesShared.unassigned : (agentById.get(key)?.name ?? routinesShared.unknownAgent),
      items: groups[key]!,
    }));
}
...
export function Routines() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: routinesPage.breadcrumb }]);
  }, [setBreadcrumbs]);
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: routinesPage.routineCreatedTitle,
        body: routine.assigneeAgentId
          ? routinesPage.routineCreatedWithAgent
          : routinesPage.routineCreatedDraft,
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: routinesPage.updateFailedTitle,
        body: mutationError instanceof Error ? mutationError.message : routinesPage.updateFailedBody,
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: routinesPage.runFailedTitle,
        body: mutationError instanceof Error ? mutationError.message : routinesPage.runFailedBody,
        tone: "error",
      });
    },
  });
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: routinesPage.defaultAgentRequiredTitle,
        body: routinesPage.defaultAgentRequiredBody,
        tone: "warn",
      });
      return;
    }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={routinesPage.emptySelectCompany} />;
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            {routinesPage.subtitle}
          </p>
        </div>
        <Button onClick={() => setComposerOpen(true)}>
          <Plus className="mr-2 h-4 w-4" />
          {routinesPage.createRoutine}
        </Button>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: routinesPage.tabRoutines },
            { value: "runs", label: routinesPage.tabRecentRuns },
          ]}
        />
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {routinesPage.routineCount((routines ?? []).length)}
            </p>
            <div className="flex items-center gap-1">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={routinesPage.sortTitle}>
                    <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{routinesPage.sort}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["updated", routinesPage.sortUpdated],
                      ["created", routinesPage.sortCreated],
                      ["lastRun", routinesPage.sortLastRun],
                      ["title", routinesPage.sortTitleField],
                      ...
                      >
                        ...
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc" ? routinesPage.sortAsc : routinesPage.sortDesc}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={routinesPage.groupTitle}>
                    <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{routinesPage.group}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["project", routinesPage.groupProject],
                      ["assignee", routinesPage.groupAgent],
                      ["none", routinesPage.groupNone],
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{routinesPage.newRoutineHeading}</p>
              <p className="text-sm text-muted-foreground">
                {routinesPage.newRoutineHint}
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={routinesShared.routineTitlePlaceholder}
                rows={1}
                value={draft.title}
                onChange={(event) => {
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{routinesShared.advancedDeliverySettings}</p>
                    <p className="text-sm text-muted-foreground">{routinesShared.advancedDeliveryHint}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">{routinesShared.concurrencyPolicyDescriptions[draft.concurrencyPolicy as keyof typeof routinesShared.concurrencyPolicyDescriptions]}</p>
                    </div>
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">{routinesShared.catchUpPolicyDescriptions[draft.catchUpPolicy as keyof typeof routinesShared.catchUpPolicyDescriptions]}</p>
                    </div>
                  </div>
                </CollapsibleContent>
              </Collapsible>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {routinesPage.afterCreationHint}
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? routinesPage.creating : routinesPage.createRoutine}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : routinesPage.createFailed}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : routinesPage.loadFailed}
          </CardContent>
        </Card>
      ) : null}
        ...
        <div>
          {(routines ?? []).length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={routinesPage.emptyNoRoutines}
              />
            </div>
          ) : (
