...
import { companyImportPage } from "../lib/i18n";
...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={companyImportPage.previewPlaceholder} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {companyImportPage.previewBinaryUnavailable}
          </div>
        )}
      </div>
    </div>
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">
            {companyImportPage.conflictTitle}
          </h3>
          <span className="text-xs text-muted-foreground">
            {companyImportPage.conflictCount(conflicts.length)}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? companyImportPage.skippedButton : companyImportPage.skipButton}
                </button>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {companyImportPage.confirmedButton}
                      </>
                    ) : (
                      companyImportPage.confirmRenameButton
                    )}
                  </button>
                )}
              </div>
...
function AdapterPickerList({
  ...
  return (
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">{companyImportPage.adapterTitle}</h3>
          <span className="text-xs text-muted-foreground">
            {companyImportPage.adapterCount(agents.length)}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
...
export function CompanyImport() {
  ...
  const localZipHelpText = companyImportPage.localZipHelp;

  useEffect(() => {
    setBreadcrumbs([
      { label: companyImportPage.breadcrumbOrg, href: "/org" },
      { label: companyImportPage.breadcrumbImport },
    ]);
  }, [setBreadcrumbs]);
  ...
  const previewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: companyImportPage.toastPreviewFailedTitle,
        body: err instanceof Error ? err.message : companyImportPage.toastPreviewFailedBody,
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: companyImportPage.toastImportCompleteTitle,
        body: companyImportPage.toastImportCompleteBody(result.agents.length),
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: companyImportPage.toastImportFailedTitle,
        body: err instanceof Error ? err.message : companyImportPage.toastApplyImportFailedBody,
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: companyImportPage.toastReadPackageFailedTitle,
        body: err instanceof Error ? err.message : companyImportPage.toastApplyImportFailedBody,
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={companyImportPage.emptySelectCompany} />;
  }
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          <h2 className="text-base font-semibold">{companyImportPage.importSourceTitle}</h2>
          <p className="text-xs text-muted-foreground mt-1">
            {companyImportPage.importSourceHint}
          </p>
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: companyImportPage.githubRepoLabel },
              { key: "local", icon: Upload, label: companyImportPage.localZipLabel },
            ] as const
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {companyImportPage.chooseZipButton}
              </Button>
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {companyImportPage.packageFileCount(
                    localPackage.name,
          ...
          <Field
            label={companyImportPage.githubUrlLabel}
            hint={companyImportPage.githubUrlHint}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              value={importUrl}
              placeholder={companyImportPage.importUrlPlaceholder}
              onChange={(e) => {
        ...
        <Field label={companyImportPage.targetLabel} hint={companyImportPage.targetHint}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">{companyImportPage.newCompanyOption}</option>
            <option value="existing">
              {companyImportPage.existingCompanyOption(selectedCompany?.name ?? "")}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={companyImportPage.newCompanyNameLabel}
            hint={companyImportPage.newCompanyNameHint}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={companyImportPage.newCompanyPlaceholder}
            />
          </Field>
        )}

        <Field
          label={companyImportPage.collisionStrategyLabel}
          hint={companyImportPage.collisionStrategyHint}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">{companyImportPage.renameOnConflict}</option>
            <option value="skip">{companyImportPage.skipOnConflict}</option>
            <option value="replace">{companyImportPage.replaceExisting}</option>
          </select>
        </Field>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending ? companyImportPage.previewingButton : companyImportPage.previewImportButton}
          </Button>
        </div>
      </div>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <span className="font-medium">
                {companyImportPage.importPreviewTitle}
              </span>
              <span className="text-muted-foreground">
                {companyImportPage.fileSelected(selectedCount, totalFiles)}
              </span>
              {conflicts.length > 0 && (
              ...
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {companyImportPage.errorCount(importPreview.errors.length)}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ? companyImportPage.buttonImporting
                : companyImportPage.buttonImportFiles(selectedCount)}
            </Button>
          </div>
